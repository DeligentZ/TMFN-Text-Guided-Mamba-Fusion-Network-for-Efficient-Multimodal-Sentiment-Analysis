...
def extract(dataset):
    ...
    input_directory_path = f'/data/zjz/workspace/MML2/data/{dataset}/Raw'
    output_directory_path = (f'/data/zjz/workspace/MML2/data/{dataset}/wav')
...
def fix_video_file(input_file, output_file):
    command = [
            ...
            for file_name in os.listdir(folder_path):
                ...
                if duration:
                    with VideoFileClip(fpath) as video:
                        if video.audio is None or video.duration != video.audio.duration:
                            print(f"Skipping due to mismatched durations or missing audio: {fpath}")
                            continue
                        ...
                        try:
                            ...
                            print(f"Failed to write video file {fpath}: {e}")
