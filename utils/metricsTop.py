...
class MetricsTop():
    def __init__(self, train_mode):
        if train_mode == "regression":
            self.metrics_dict = {
                ...
                'EATD':self.__eval_eatd_regression,
                'UR_FUNNY': self.__eval_urfunny_regression
            }
        else:
            self.metrics_dict = {
    ...
    def __eval_sims_regression(self, y_pred, y_true):
        ...
        # five classes{[-1.0, -0.6], (-0.6, -0.2], (-0.2, 0.2], (0.2, 0.6], (0.6, 1.0]}
        ms_5 = [-1.01, -0.6, -0.2, 0.2, 0.6, 1.01]
    ...
    def __eval_eatd_regression(self, y_pred, y_true):
        test_preds = y_pred.view(-1).cpu().detach().numpy()
        test_truth = y_true.view(-1).cpu().detach().numpy()
        test_preds = np.clip(test_preds, a_min=-1., a_max=1.)
        test_truth = np.clip(test_truth, a_min=-1., a_max=1.)

        # two classes{[20.0, 49.0], (50, 80]}
        ms_2 = [20.01, 50, 80.01]
        test_preds_a2 = test_preds.copy()
        test_truth_a2 = test_truth.copy()
        for i in range(2):
        ...
        # three classes{[20.0, 50], (50, 59], (60, 69],(70,80]}
        ms_4 = [20.01,50,60,70,80.01]
        test_preds_a4 = test_preds.copy()
        test_truth_a4 = test_truth.copy()
        for i in range(4):
            test_preds_a4[np.logical_and(test_preds > ms_4[i], test_preds <= ms_4[i + 1])] = i
        for i in range(4):
            test_truth_a4[np.logical_and(test_truth > ms_4[i], test_truth <= ms_4[i + 1])] = i
        ...
        mult_a2 = self.__multiclass_acc(test_preds_a2, test_truth_a2)
        mult_a4 = self.__multiclass_acc(test_preds_a4, test_truth_a4)
        f_score = f1_score(test_truth_a2, test_preds_a2, average='weighted')

        eval_results = {
            "Mult_acc_2": round(mult_a2, 4),
            "Mult_acc_3": round(mult_a4, 4),
            "F1_score": round(f_score, 4),
            "MAE": round(mae, 4),
            "Corr": round(corr, 4),  # Correlation Coefficient
        }
    ...
    def __eval_urfunny_regression(self, y_pred, y_true):
        test_preds = y_pred.view(-1).cpu().detach().numpy()
        test_truth = y_true.view(-1).cpu().detach().numpy()

        test_preds_sigmoid = 1 / (1 + np.exp(-test_preds))

        binary_preds = (test_preds_sigmoid > 0.5).astype(int)
        binary_truth = test_truth.astype(int)
